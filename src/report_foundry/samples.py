...
def _sources(repo_meta: dict[str, dict[str, object]], observed_at: str) -> list[SourceObservation]:
    ...
    for path, source_id, title in [
        (Path("docs/OSS_REPORT_GENERATION_LANDSCAPE.md"), "src_repo_oss_landscape", "Report Foundry repo — OSS landscape doc"),
...
def _facts(repo_meta: dict[str, dict[str, object]]) -> list[EvidenceFact]:
    ...
    return [
        EvidenceFact(fact_id="fact_typesetting_stack", subject="Report Foundry renderer strategy", predicate="formatting_prior_art", value="Typst, Quarto, Pandoc, WeasyPrint, Paged.js, and Playwright-class tools cover existing document rendering routes.", source_id="src_repo_oss_landscape", quote="The OSS landscape doc lists Typst, Quarto, Pandoc, SILE, WeasyPrint, Vivliostyle, Paged.js, Playwright, and Puppeteer as formatting / PDF / publishing engines.", locator="docs/OSS_REPORT_GENERATION_LANDSCAPE.md#formatting--pdf--publishing-engines"),
        ...
        EvidenceFact(fact_id="fact_report_foundry_ownership", subject="Report Foundry architecture boundary", predicate="design_principle", value="Report Foundry should own evidence/provenance/orchestration/QA and delegate layout/citation/exhibit machinery.", source_id="src_repo_oss_landscape", quote="The foundry owns validation, orchestration, provenance, and QA. It should delegate formatting and rendering to established engines.", locator="docs/OSS_REPORT_GENERATION_LANDSCAPE.md#architecture-implication"),
